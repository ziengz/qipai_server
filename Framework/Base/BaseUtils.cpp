...
bool BaseUtils::encodeBase64(std::string &base64, const char *input, int len) {
    ...
    int ret = EVP_EncodeBlock(
        buf, reinterpret_cast<const unsigned char *>(input), len);
    if (ret > 0)
...
bool BaseUtils::decodeBase64(const std::string &input, std::string &output) {
    ...
    int ret = EVP_DecodeBlock(buf, (const unsigned char *)input.data(),
                              (int)output_len);
...
char *BaseUtils::strdup(const char *str) { return ::strdup(str); }